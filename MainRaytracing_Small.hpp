...
struct color_s_char {
	...
	color_s_char(const color_s & c) : r(toByte(c.r)), g(toByte(c.g)), b(toByte(c.b)){}
	color_s_char(){}
};
...
void receiveResult() {
	...
	color_s_char * buffer=NULL; uint size;
	...
	for (uint i = indexIni; i < indexEnd; ++i) {
		const color_s_char & c = buffer[i-indexIni];
		image->setPixelChar(i/IMAGE_WIDTH, i%IMAGE_WIDTH, c.r, c.g, c.b);
	}
